...
- [x] Cloudflare **dev** worker `momentum-tennis-dev` created, first deploy done, `deploy.site_url` recorded. Still open: the live worker `momentum-tennis` (branch `deploy/live`), and **Cloudflare Access on the dev host is NOT applied and cannot be** while the site lives on `*.workers.dev`: an Access application attaches to a hostname in a zone on the account, and that zone is Cloudflare's. Protecting dev needs a custom domain first — `docs/OPERATIONS.md` §3. `config/dev.yaml` now records `access_protected: false` with the reason
