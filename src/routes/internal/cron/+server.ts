 ...
 * Until CRON_SHARED_SECRET is set for an environment this endpoint answers 503, not 500.
 */
export const POST: RequestHandler = async ({ request }) => {
	const secret = secretOr503('CRON_SHARED_SECRET', 'Cron');
	if (!authorizeCron(request.headers.get('authorization'), secret)) error(401, 'Unauthorized');
