 ...
 * a load function that renders for a user. Demands SUPABASE_SECRET_KEY at first use.
 */
export function createAdminSupabase(): SupabaseClient<Database> {
	const cfg = getConfig();
	client ??= createClient<Database>(cfg.supabaseUrl, requireSecret(cfg, 'SUPABASE_SECRET_KEY'), {
